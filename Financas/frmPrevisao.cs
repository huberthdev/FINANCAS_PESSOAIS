...
namespace Setup.Financas
{
    public partial class frmPrevisao : Form
    {
        ...
        public void CarregarPrevisao()
        {
            ...
            //
            sql += "UNION SELECT C.CLASSE, ' Receita Creditada' AS TIPO, B.DIA, B.VALOR AS ORCADO, SUM(A.VALOR) AS REALIZADO, IIF((SUM(A.VALOR) - B.VALOR) > 0, 0, (ABS(SUM(A.VALOR) - B.VALOR))) AS DESVIO, IIF(SUM(A.VALOR) >= B.VALOR, 1, 0) AS STATUS, 'B' AS ORD, B.OBS, B.CHAVE ";
            ...
            sql += "IIF(B.VALOR IS NULL, 0, B.VALOR) - ABS(SUM(A.VALOR)) AS DESVIO, IIF(ABS(SUM(A.VALOR)) >= IIF(B.VALOR IS NULL, 0, B.VALOR), 1, 0) AS ST, 'B' AS ORD, B.OBS AS OB, ";
